...
const approveJoinRequest = async (req, res) => {
    try {
        ...
        group.joining_requests = group.joining_requests.filter(id => id.toString() !== userId);
        await group.save();
...
const rejectJoinRequest = async (req, res) => {
    try {
        ...
        if (group.admin.toString() !== req.user._id.toString()) {
            return res.status(403).json({ error: "Only admin of group can reject join requests" });
        }
    ...
    try {
        ...
        if (group.admin.toString() === req.user._id.toString()) {
            return res.status(403).json({
                message: "Group admin cannot leave the group. Please transfer admin rights first.",
            });
        }
